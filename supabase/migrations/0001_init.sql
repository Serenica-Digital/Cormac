-- Cormac app-owned operational schema. Ported from v0 (archive/supabase/
-- migrations/0001_init.sql), proven live against real Postgres before the
-- archive; SQL semantics unchanged.
--
-- These are the ONLY fixed tables. Business objects are contract-defined data
-- that lives in business_records, never their own tables (requirements.md:
...
create table public.workspaces (
  ...
  name text not null,
  -- Per-workspace write policy. Skeleton ships confirm_each.
...
-- ---------------------------------------------------------------------------
-- Memberships: which user has which role in which workspace.
...
-- ---------------------------------------------------------------------------
-- Contract versions: the published, versioned semantic contract.
...
-- ---------------------------------------------------------------------------
-- Business records: the JSONB-first hybrid store.
...
-- ---------------------------------------------------------------------------
-- Source messages: the inbound context every proposal traces back to.
-- ---------------------------------------------------------------------------
...
-- Nothing is written from a proposal until it is applied by the control plane
-- (requirements.md: one pipeline, control plane is the only writer).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Audit events: append-only, with before/after and source link. Append-only is
-- enforced by trigger below, so it holds even against the service role.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Explicit PostgREST grants (new in v2). The Supabase CLI/cloud default
-- flipped 2026-05-30: new tables in public are no longer implicitly exposed to
-- the Data API roles, so the grants v0 got for free are now explicit. The
-- posture is deliberate: the service role (the control plane, the only writer)
-- gets CRUD; authenticated users get SELECT only, with RLS (0002) filtering
...
grant select, insert, update, delete on public.business_records to service_role;
grant select, insert, update, delete on public.source_messages to service_role;
...
grant select on public.source_messages to authenticated;
