...
describe.skipIf(!ready)('compiled workspace context', () => {
  let service: Db;
  let app: AppContext;
  ...
  beforeAll(async () => {
    ...
    const ws = await service.from('workspaces').insert({ name: `ctx-${randomUUID()}` }).select('id').single();
    workspaceId = resources.workspace(ws.data!.id as string);
    const user = await service.auth.admin.createUser({
      email: `ctx-${randomUUID()}@test.local`,
