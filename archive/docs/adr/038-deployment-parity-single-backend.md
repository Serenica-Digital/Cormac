...
**Status:** Accepted, with the Terra-packaging and local-rehearsal sections superseded by ADR-039 (a verified research pass over the `juno-fx` source corrected them). What stands: managed Supabase as the single backend, the Kubernetes consumption model, the Hermes runtime operational shape, and the two image lanes. What ADR-039 corrects: the "chart is the plugin in place / no `plugins/` wrapper" packaging (section 6), the "k3d is the local environment / byte-identical to Juno" rehearsal (section 1), and the cert-manager/ClusterIssuer and two-track helm-fallback assumptions (section 3). Proven live at the time of writing: the whole backend runs on local k3d from the same Helm charts, the walking skeleton passes 11/11 with real Hermes, and the managed `APP_ENV=dev` / ES256 posture boots, authenticates, and reads managed Postgres (this closes ADR-035 open item 1). The Juno *cluster* deploy itself is gated on the onboarding session (#15).
**Date:** 2026-06-13
