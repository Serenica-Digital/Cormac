...
**Status:** Accepted. The managed `APP_ENV=dev` / ES256 posture is proven live (seed, fail-closed boot, JWKS verification of a managed-issued ES256 token, and a managed Postgres read against the managed dev project), which closes ADR-035 open item 1. CI is green on `f17aaa6`. The Juno cluster deploy itself stays gated on the onboarding session (#15). **Superseded and consolidated by ADR-037 (the single variable set and the guard deletions) and ADR-038 (the managed single backend, the store deletions, and the Terra packaging); kept as the granular record of the managed collapse.**
**Date:** 2026-06-13
