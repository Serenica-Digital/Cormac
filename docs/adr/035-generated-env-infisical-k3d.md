...
**Status:** Accepted for the generation model and the Infisical/Helm/k3d direction. The live k3d walking-skeleton proof against managed dev Supabase is gated on Infisical provisioning and is recorded as unproven until it runs (see Open items). Resolves ADR-034 Open item 2 (the secret backend).
**Date:** 2026-06-13
...
1. **The live walking-skeleton proof in k3d is gated and unproven.** It needs the managed dev Supabase service-role key, an Anthropic key, and the runtime/MCP tokens in Infisical, plus the machine-identity bootstrap secret, so ESO can synthesize `cormac-secrets` and the api can boot under `APP_ENV=dev`. The credential-free mechanics proof (worker and hermes-runtime on k3d) runs without them; the full capture-to-audit smoke does not. Status is recorded honestly until it runs.
2. **Compose retirement waits on that proof.** `docker/compose.yaml` and its scripts are removed, and the M1 pane runbook migrated to k3d, only after the live proof passes, coordinated with the in-flight M1 work.
...
4. **CI machine-identity rollout.** The single Infisical token in GitHub Actions is added so existing guards stay green; the migration of CI's current Supabase-from-`supabase status` env to Infisical-injected values is staged, not big-bang.
