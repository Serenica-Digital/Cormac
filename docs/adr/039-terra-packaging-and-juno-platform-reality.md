...
**Status:** Accepted. The packaging restructure is landed and CI-verified (the five charts live under `plugins/<workload>/`, lint and template green, `check:env` clean, the bundle rewritten to the real Terra schema). The Juno-shaped kind rehearsal that proves the ArgoCD-synced path, field injection, ESO, TLS, and pull-secret ordering is the next step and is tracked as open items below; until it runs, the platform-native deploy is designed-not-proven.
**Date:** 2026-06-14
...
The five workload charts move from `deploy/helm/<workload>/` to `plugins/<workload>/` at the repo root. Each `plugins/<workload>/` directory is, at once, a Terra plugin (it carries `terra.yaml` beside `Chart.yaml`/`values.yaml`/`templates/`) and a plain Helm chart (`terra.yaml` is inert to Helm). There is no `deploy/helm` and no `plugins/` wrapper or mirrored copy: one directory per workload, one source of truth. The Cormac repo is the Terra Source; Terra scans `plugins/*/terra.yaml` at its root.

The bundle moves to `bundles/cormac.yaml` at the repo root and is rewritten to the verified schema: plugins referenced by their display `name`, shared bundle fields fanned into each plugin via `values: {<plugin_field>: {reference: <bundle_field>}}`. `resource_id` is treated as deprecated; `name`/`tags`/`fields` are load-bearing. Per-workload ingress hosts are surfaced as separate bundle fields because a reference maps a value, it does not template `api.` + base.
...
1. **The Juno-shaped kind rehearsal** (the blocking dress-rehearsal checklist): stand up kind + ArgoCD + ingress-nginx + cert-manager, deploy the five plugins through the ArgoCD-synced path, prove the GHCR pull-secret-before-workload ordering, install ESO + Infisical and confirm `cormac-secrets` lands, issue real TLS for a pane host, prove the noauth webhook ingress, run the capture-to-audit smoke against managed Supabase, and run an egress reachability check. Retire the k3d scripts when the kind path lands.
2. **Field-to-values injection: flat vs nested.** The Terra contract fixes that a field `name` injects into a chart value, and every proven plugin uses flat top-level keys, while our charts read nested values (`image.tag`, `ingress.host`). Whether Terra resolves the dotted path or needs flat top-level keys is verified at the rehearsal; if flat is required, flatten the operator-set values across the charts.
3. **Private-repo Source registration and auth.** How Terra/ArgoCD authenticates to our private Source repo; the public docs only show public Sources.
