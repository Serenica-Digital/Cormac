...
1. **The Juno-shaped kind rehearsal.** Base substrate done (2026-06-15, [../runbooks/juno-local-rehearsal.md](../runbooks/juno-local-rehearsal.md)): kind + ArgoCD + ingress-nginx + cert-manager + a ClusterIssuer, all five charts apply and pass ingress-nginx admission, cert-manager issues TLS, the pull-secret-before-workload requirement is demonstrated, and full Genesis + Terra runs locally with no credentials. Remaining: the credentialed Cormac-on-Genesis path (ArgoCD/Terra reading the private repo, the GHCR pull, ESO -> `cormac-secrets`, the bundle launch through the Genesis UI), the capture-to-audit smoke against managed Supabase, and an egress reachability check. Retire the k3d scripts once a committed kind-based dev/rehearsal script replaces `pnpm dev`.
2. **Field-to-values injection: flat vs nested.** The Terra contract fixes that a field `name` injects into a chart value, and every proven plugin uses flat top-level keys, while our charts read nested values (`image.tag`, `ingress.host`). The rehearsal showed a dotted Helm `--set image.tag=X` does reach the nested value, which is the semantics ArgoCD `helm.parameters` reuse, so nested values resolve if Terra injects that way. The residual is only if Terra uses a literal-key `valuesObject`; settle it against the running local Terra by registering a Source, otherwise confirm at onboarding. If flat is required, flatten the operator-set values across the charts.
