...
| --- | --- | --- | --- |
| 1 Unit | Pure logic, no I/O (no DB, no socket) | co-located `*.test.ts` beside the code, in `packages/*/src` and `apps/*/src` | anywhere, fast, every PR; no Docker |
| 2 Integration | Real Postgres and/or a real socket, multiple components wired | root `tests/*.test.ts` | needs local Supabase; every PR in CI |
...
The rule that was missing and is now explicit: **Tier 1 unit tests live next to the code they test; Tier 2 integration tests live in the root `tests/` directory.** A test that needs `createServiceClient`, a workspace, or a token is Tier 2 and belongs in `tests/`. A test that imports a pure function and asserts its output is Tier 1 and stays beside the source. `apps/api/src/server.test.ts` is the one boundary case: it is a Tier 1 test (no DB, `fastify.inject` only) that proves server-boundary controls, and it stays with the server it builds.
...
1. **Contract and field gates (Tier 1).** The agent-editability and type gate and the sensitivity helpers, the cheapest and most load-bearing logic tests. `packages/contract/src/validate.test.ts` (no human-only field, no wrong type, no bad enum, no unknown field; create requires required fields); `packages/contract/src/redact.test.ts` (sensitive values stay out of the model context and are masked in logs); `apps/api/src/adapter/runtime.test.ts` (malformed runtime output is rejected at the adapter, never written).

2. **Trust invariants (Tier 2, must never regress).** These prove the boundary, not a feature. Tenant isolation, a user in workspace B cannot read or write workspace A's rows (`tests/isolation.test.ts`); the control plane is the only writer (the isolation write-denied case; the runtime gets no DB credentials in [compose](../../docker/compose.yaml)); append-only audit (the isolation append-only case); the RBAC matrix, every protected endpoint times every role (`tests/rbac-matrix.test.ts`); authn, every protected endpoint rejects a missing or invalid JWT.

3. **Pipeline and knowledge spine (Tier 2, against a real DB).** The full capture → validate → confirm → write → audit thread (`tests/pipeline-integration.test.ts`); atomic apply and all-or-nothing rollback (`tests/atomic-apply.test.ts`); the MCP tool surface, auth, allowlist, and the write gate (`tests/mcp-tools.test.ts`); the contract publish gate, server-authoritative, atomic, audited, invalid and direct-call refused (`tests/contract-publish.test.ts`); the learned-knowledge gate, proposed-is-invisible, approve activates and audits, revoke reopens the slot, stale items refused (`tests/learning-lifecycle.test.ts`); the compiled context prefix, only active gated items, aliases resolved, sensitive values excluded, byte-stable (`tests/context-compile.test.ts`); workspace purge, deliberate and service-role-only (`tests/workspace-purge.test.ts`).
...
5. **Security and abuse (Tier 1 and Tier 2).** Secrets never appear in source or the browser bundle (CI `check:secrets`); CORS is an allowlist and 5xx error detail is withheld (`apps/api/src/server.test.ts`); the SSE capture surface enforces the same authz chain as `/capture` and never leaks error detail (`apps/api/src/routes/capture-stream.test.ts`, `tests/sse-stream-integration.test.ts`); injection, contract-gate-as-backstop, and per-surface authenticity tested per surface as it lands.

6. **Non-functional (Tier 2 and manual).** Runtime latency budget for the capture round-trip; failure injection, runtime down and mid-way apply failure (`tests/atomic-apply.test.ts`); the backup/restore drill (manual, pending).

## The shared integration harness

Every Tier 2 suite stands up the same shape: a workspace, an owner user, a membership, an active contract, sometimes a token. That setup lives once, in [tests/helpers.ts](../../tests/helpers.ts), not copied per file:
...
| Criterion | Status |
| --- | --- |
| Workspace scoping on all tenant data | met; `tests/isolation.test.ts` (register 1) |
...
| Control plane verifies JWT + workspace role on protected endpoints | met; `tests/rbac-matrix.test.ts` (register 9, 10), HS256 path in CI / ES256 manual |
| Agent runtime has no write credentials | met (compose; register 3) |
| Agent tools allowlisted by workspace | met + tested; `tests/mcp-tools.test.ts` (register 20) |
...
| Knowledge layer is gated and compiled control-plane-side | met + tested; `tests/learning-lifecycle.test.ts`, `tests/context-compile.test.ts` (register 25, 26) |
...
- **After local Supabase:** all Tier 2 tests. They skip cleanly and loudly without a database (the `requireSupabaseEnv` warning), following the pattern in [tests/isolation.test.ts](../../tests/isolation.test.ts).
