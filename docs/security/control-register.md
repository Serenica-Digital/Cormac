...
| 28 | Per-change proposal decisions keep the write gate whole: only the kept subset is validated and applied atomically; dropped changes write nothing and land on the audit trail | `decideProposal` `keep` parameter ([apply.ts](../../apps/control-plane/src/pipeline/apply.ts)) behind `requireCapability('approve_proposal')`; the kept subset re-validated via `validateProposalAgainstContract` and applied by the same atomic `apply_proposal`; drops audited as `proposal_changes_dropped` with the dropped changes recorded | `apps/control-plane/tests/partial-decision.test.ts` | [identity-and-access.md](identity-and-access.md) | Verified |
| 29 | Agent learning is propose-only: a typed, contract-validated fact enters the agent's context only after a human decision, and the decision is audited | `POST /agent/learning` ([agent.ts](../../apps/control-plane/src/routes/agent.ts)) holds `pending` rows validated against the active contract (two typed kinds only, no free text); the decision route rides `requireCapability('approve_proposal')` and writes a `learning_approved`/`learning_rejected` audit event; only `approved` rows compile into the model context ([context.ts](../../apps/control-plane/src/pipeline/context.ts)) | `apps/control-plane/tests/learned-knowledge.test.ts` | [agent-security-and-ai-data-handling.md](agent-security-and-ai-data-handling.md) | Verified |
