...
- Environment and secrets are one governed contract: a single manifest ([@cormac/config](../../packages/config/src/manifest.ts)) declares every variable once, classified secret or non-secret, and every workload validates its slice at boot (ADR-034). None are hard-coded. `.env.example` and each chart's `env`/`secretEnv` are generated from the manifest by `pnpm gen:env`; the manifest is the only declaration site (ADR-035).
...
- Secret values have one authority: Infisical (ADR-035). Host tooling runs under `infisical run`, so no real `.env` of values sits on disk; CI holds one machine-identity token; the cluster's `cormac-secrets` is synthesized by the External Secrets Operator ([deploy/eso/](../../deploy/eso/)). Every read is logged in Infisical.
...
- **Managed store chosen; live sync not yet proven.** Infisical is the secret authority and the ESO wiring is scaffolded ([deploy/eso/](../../deploy/eso/), ADR-035), but the live synthesis of `cormac-secrets` in a cluster is gated on the operator provisioning the Infisical project, secret values, and machine identity. Until that runs it is recorded as unproven (ADR-035 open item 1).

## Rotation

- Rotate a value once in Infisical; host tooling (`infisical run`), CI, and the cluster (ESO refresh, 1h) pick it up (ADR-035, [deploy/eso/README.md](../../deploy/eso/README.md)).
