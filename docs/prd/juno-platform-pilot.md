...
The mapping between how this repo already works and how Juno talks about it. The short version: the Helm charts run locally on k3d as the rehearsal, Juno is the venue, and the images are identical in both (ADR-038).
...
> **Resolved against the live juno-fx docs and repos on 2026-06-13** (these were "unknown" on 2026-06-10; ADR-038 builds on them, and the Helm charts that encode it live in [../../deploy/helm/](../../deploy/helm/)): **#1** our own charts render a ClusterIP Service and no Ingress, so `clusterip` needs no dependency on PR #557 (still open). **#3** secrets are standard k8s Secrets injected via `secretKeyRef`, and Juno now advertises External Secrets Operator integration (Vault / AWS Secrets Manager / Azure Key Vault). **#6** Juno has no native recycle, so the Hermes chart ships a CronJob `rollout restart` plus a memory limit (the upstream leak #25315 is unfixed in v0.16.0). **#10** the security page now claims pod-to-pod mTLS. **#12** `cert-manager` is an available Terra plugin, so the pane chart sets `host` + `tls` + a `cluster-issuer` annotation. The residual asks are now operational confirmations: a `ClusterIssuer` exists and DNS for the pane domain can be delegated (#12, else CDN); an ESO `SecretStore` is provisioned or we ship plain Secrets, and who can read them back (#3); CronJob RBAC in our namespace (#6); hostname stability across updates (#4); and which Hermes build the pilot runs.
