...
- **The env, secrets, and deployment foundation landed (ADR-037/038), then corrected to the platform reality (ADR-039).** One generated env contract from a single manifest, Infisical the secret authority synced via ESO, and the whole backend proven against managed Supabase (everywhere except CI). The Terra packaging is now CI-verified: the five charts live under `plugins/` (each a Terra plugin and a plain Helm chart at once), the bundle at `bundles/cormac.yaml`, deploy via Terra/ArgoCD with `helm install` as a portability escape hatch. ESO, cert-manager, and cluster egress are Cormac-owned (Juno ships none). This is the deployment groundwork M5 builds on; the Juno-shaped kind rehearsal and the live cluster deploy are gated on the onboarding session (#15).
