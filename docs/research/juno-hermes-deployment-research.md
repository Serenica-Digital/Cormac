...
- **Source visibility.** What is public and current (pushes within 48h of 2026-06-10): the deployment Helm charts (Orion-Deployment, Genesis-Deployment), the full Terra plugin repo including its feature branches and open PRs, the Helios workstation images, and the infra tooling (juno_k3s, EKS-Deployment, K8s-Playbooks, Juno-Bootstrap, Airgap-Bundler, Orion-Go/Orion-Py clients). What is closed: the application source of the management plane itself (Genesis, Hubble, Kuiper, Rhea, Titan, the Terra service). We can inspect how the platform is deployed and everything it deploys, and we can watch the roadmap through PRs and the 51 open issues, but the control-plane internals are container images only. (Verified)
...
- **Build-from-repo runtime plugins exist, and they are public after all.** The demo showed JavaScript (Vite), Python, and Go runtime workload plugins configured with repository, branch, build command, run command, listen port, an auth toggle, and optional GPU. A 2026-06-10 follow-up found them in public PR #557 (branch `556-runtime-environments`): `runtime-js`, `runtime-python`, `runtime-go`, `runtime-cpp`, each taking `git_url`/`git_ref` (or `source_path` for code on a shared mount), `build_command`, `run_command`, `port`, `gpu`, and the `network_mode` select above. "Not on our main branches" meant public feature branches, not private repos. The pilot likely registers this branch as a Terra Source; confirm with the team.
...
- Ingress is NGINX with path-based routing (`/<plugin>/<workload-name>/` on one cluster host). URLs are stable while the workload name and cluster host are stable; whether the pilot cluster hostname is stable across redeployments is unknown. **Update 2026-06-13:** `cert-manager` is now an available Terra plugin (v1.19.1), so a workload chart we author can set its own `host` + `spec.tls` + a `cluster-issuer` annotation rather than relying on the shared cluster host. Cormac's pane chart does exactly this (ADR-034); confirm a `ClusterIssuer` exists and DNS can be delegated, else front the pane with a CDN.
...
- **Zero-ingress workloads: solved in an unmerged public PR.** The first sweep found no documented way to deploy a workload with no public route, and every merged plugin creates an Ingress. A 2026-06-10 follow-up found PR #557 (branch `556-runtime-environments`, updated 2026-06-09): the new runtime plugins expose a `network_mode` select with options `ingress-auth`, `ingress-noauth`, `clusterip`, and `nodeport`. `clusterip` is exactly the cluster-internal, no-public-route mode the Hermes product runtime requires, and `ingress-noauth` is the public-webhook mode the control plane API requires. Unmerged as of 2026-06-10, so confirm at onboarding that the pilot cluster carries it. **Update 2026-06-13:** PR #557 is still open on `main`, but it is no longer a blocker. Cormac deploys CI-built images as its own workload charts (not Juno's runtime plugins), and those charts render a ClusterIP Service with no Ingress for the runtime and worker, so `clusterip` does not depend on PR #557 at all (ADR-034).
- **The mTLS claim on the security page is not substantiated by the manifests.** No service mesh is deployed; cross-node traffic is WireGuard-encrypted at the k3s layer, which is not pod-to-pod mTLS. Do not cite mTLS in the client security packet until Juno explains the mechanism in writing. **Update 2026-06-13:** the security page now states pod-to-pod mTLS explicitly ("Workload-to-workload communication is encrypted and authenticated via mTLS"), but the manifests still show no service mesh, so the gap stands: get the concrete mechanism in writing before citing it.
