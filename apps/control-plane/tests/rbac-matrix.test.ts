...
import type { Role } from '../src/shared.js';
import { buildServer } from '../src/server.js';
import { loadConfig } from '../src/config.js';
import { mintToken, requireSupabaseEnv, TestResources } from './helpers.js';

/**
 * The RBAC matrix (ported from v0, minus the learning routes deferred with
 * #35): every protected endpoint, for every role, allows exactly its
 * capabilities and rejects the rest, and an unauthenticated or non-member
 * request is rejected. Uses fastify.inject and real minted tokens. Skips
...
describe.skipIf(!env.ready)('RBAC matrix', () => {
  ...
  it('gates workbook upload on publish_contract, like the contract itself', async () => {
    const allowed = new Set<Role>(['owner', 'agent_admin']);
    for (const role of ROLES) {
      const res = await server.inject({
        method: 'POST',
        url: `${base()}/workbook`,
        headers: bearer(token[role]!),
        payload: { name: `wb-${role.replace('_', '-')}`, profile: { sheets: [] } },
      });
      if (allowed.has(role)) {
        expect(res.statusCode, role).toBe(200);
      } else {
  ...
  it('denies capture for read_only and admits it for others', async () => {
    ...
    // A member passes authz; with no runtime configured the handler answers
    // 503, which still proves authorization let it through (not 401/403).
