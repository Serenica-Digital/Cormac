...
/**
 * The shared integration harness, ported from v0
 * (archive/tests/integration/helpers.ts). Every DB-backed suite stands up the
 * same shape, gates on the same env, and mints tokens the same way. Env comes
 * from `infisical run` (or the shell), never from a repo .env file.
 */
  ...
  if (!ready && !skipWarned) {
    ...
    console.warn(
      '\n  ⚠ SKIPPING integration tests: SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY unset.' +
        '\n    Run `pnpm db:start` (needs Docker) and rerun under `infisical run` to exercise them.\n',
    );
  }
 ...
 * with the issuer and `aud=authenticated` that `authenticate` expects.
 * NOTE: this exercises the HS256 verification branch (what local Supabase
 * uses); production verifies ES256 against the JWKS, which the #66 managed
 * Supabase closing step proves live.
 */
 ...
 * cleanup. Suites add their own records on top of this.
 */
 ...
 * in afterAll, so test runs stop accumulating residue. Workspaces go through
 * the service-role-only purge_workspace path (migration 0005), the one
 * deletion the append-only audit trigger sanctions; auth users are deleted
 * after their workspace rows are gone.
 */
